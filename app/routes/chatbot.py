...
def chatbot(user_id):
    ...
    current_app.logger.info(f"User ID {user_id}의 요청을 받았습니다: {query}")
    ...
    # 개인화된 응답 생성
    personalized_response = llm_service.generate_personalized_response(user_id, query)
    current_app.logger.info(f"LLM response: {personalized_response}")
    
    # AI 응답을 DB에 저장
    try:
        save_ai_response(user_id, query, personalized_response)
        current_app.logger.info(f"User ID {user_id}의 응답이 성공적으로 저장되었습니다.")
    except Exception as e:
        current_app.logger.error(f"User ID {user_id}의 응답 저장 실패: {e}")
